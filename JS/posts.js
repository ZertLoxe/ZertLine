...
fetch(directLink)
  ...
  .then((posts) => {
    ...
    for (let i = Object.keys(posts).length - 1; i >= 0; i--) {
      ...
      outPostList +=
        "<a class='posts-card' id='post-" +
        postKey +
        "' href='posts.html?post=" +
        postKey +
        "'><div class='post-card-content'><h1 class='post-card-title'>" +
        posts[postKey].postTitle +
        "</h1><p class='post-card-about'>" +
        posts[postKey].postAbout +
        "</p></div></a>";
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var i = 0; i < postVarLinkPart1.length; i++) {
      ...
      if (postVarLinkPart2[0] === "post") {
        postVarLink = decodeURIComponent(
          postVarLinkPart2[1].replace(/\+/g, " ")
        );
        ...
        fetch(directLink)
          ...
          .then((post) => {
            if (postVarLink in post) {
              getElement("posts-header").innerHTML =
                post[postVarLink].postTitle;
              ...
              for (let i = 0; i < post[postVarLink].postElements.length; i++) {
                if (
                  JSON.stringify(
                    post[postVarLink].postElements[i].postElementStructure
                  ) !== JSON.stringify(["false", "0", "0"])
                ) {
                  let outTitle = "";
                  let outMedia = "";
                  let outText = "";
                  ...
                  if (
                    post[postVarLink].postElements[i]
                      .postElementStructure[1] === "2"
                  ) {
                    for (
                      let j = 0;
                      j < post[postVarLink].postElements[i].media.length;
                      j++
                    ) {
                      if (
                        post[postVarLink].postElements[i].media[j][0] === "I"
                      ) {
                        outMedia +=
                          "<img src='https://i.ibb.co/" +
                          post[postVarLink].postElements[i].media[j][1] +
                          "' id='post-element-image' class='media-item' loading='lazy'>";
                      } else {
                        outMedia +=
                    ...
                    post[postVarLink].postElements[i]
                      .postElementStructure[1] === "1"
                  ) {
                    if (post[postVarLink].postElements[i].media[0] === "I") {
                      outMedia =
                        "<img src='https://i.ibb.co/" +
                        post[postVarLink].postElements[i].media[1] +
                        "' id='post-element-image' class='media-item' loading='lazy'>";
                    } else {
                      outMedia =
                        "<iframe id='post-element-video' class='media-item' src='https://www.youtube.com/embed/" +
                  ...
                  if (
                    post[postVarLink].postElements[i]
                      .postElementStructure[2] === "2"
                  ) {
                    for (
                      let j = 0;
                      j < post[postVarLink].postElements[i].text.length;
                      j++
                    ) {
                      outText +=
                        "<p>" +
                        post[postVarLink].postElements[i].text[j] +
                        "</p>";
                    }
                  } else if (
                    post[postVarLink].postElements[i]
                      .postElementStructure[2] === "1"
                  ) {
                    outText =
                  ...
                  if (post[postVarLink].postElements[i].elementShape === "RM") {
                    ...
                    outPostElements +=
                      "<div id='post-element-part-LM'><div id='post-element-media-holder'>" +
                      outMedia +
                      "</div><div id='post-element-title-text-holder'>" +
                      outTitle +
                      "<div id='post-element-text-holder'>" +
                      outText +
                    ...
                    outPostElements +=
                      "<div id='post-element-part-AG'>" +
                      outTitle +
                      "<div id='post-element-media-holder' class='AG-post-element-media-holder'><button class='AG-arrow-left-button' data-slider=" +
                      AGDataSlider +
                      " data-direction='left'></button><svg id='AG-arrow-left' class='AG-arrow-left' viewBox='0 0 650.34 1000'><path d='M618.4,926.79l-41.27,41.27c-42.58,42.58-111.62,42.58-154.2,0L31.93,577.06c-42.58-42.58-42.58-111.62,0-154.2L422.87,31.93c42.58-42.58,111.62-42.58,154.2,0l41.31,41.31c42.58,42.58,42.58,111.62,0,154.2l-234.01,234.01c-21.29,21.29-21.29,55.81,0,77.1l234.05,234.05c42.58,42.58,42.58,111.62,0,154.2Z'/></svg><div data-slider=" +
                      AGDataSlider +
                      " class='AG-post-element-media-holder-slider'>" +
                      outMedia +
                      "</div><button class='AG-arrow-right-button' data-slider=" +
                      AGDataSlider +
              ...
              if (post[postVarLink].time[1] !== null) {
                outPostEditTime =
                  "<div id='edit-time'><p>Post edited on " +
                  moment(post[postVarLink].time[1])
                    .tz(targetTimeZone)
              ...
              let out =
                "<div id='post-elements-holder'>" +
                outPostElements +
                "</div><div id='post-time'><div id='release-time'><p>Post released on " +
                moment(post[postVarLink].time[0])
                  .tz(targetTimeZone)
                  .format("(MMM-D-YYYY) (h:mm a)") +
                "</p></div>" +
                outPostEditTime +
              ...
              getElement("posts-footer").innerHTML =
                "<p>the '" + postVarLink + "' post was not found :(</p>";
...
const afterPostLoad = async () => {
  try {
    ...
    const sliders = document.querySelectorAll(
      ".AG-post-element-media-holder-slider"
    );
    ...
    sliders.forEach(function (slider) {
      ...
      document.addEventListener("click", function (event) {
        ...
        if (
          event.target.classList.contains("AG-arrow-left-button") ||
          event.target.classList.contains("AG-arrow-right-button")
        ) {
